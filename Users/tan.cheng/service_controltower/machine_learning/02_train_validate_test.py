...
def _params_for_variant(variant: str, selected_cv: dict) -> dict:
    override = {}
    if variant in selected_cv:
        override = selected_cv[variant].get("param_override") or {}
    return override
...
def run() -> None:
    ...
    for variant in config.MODEL_VARIANTS_TO_RUN:
        ...
        override = _params_for_variant(variant, selected_cv)
        ...
        val_prob = model.predict_proba(prepared.X_validation)[:, 1]
        test_prob = model.predict_proba(prepared.X_test)[:, 1]

        val_free = threshold_free_metrics(y_validation, val_prob)
        threshold_free_rows.append({"model_variant": variant, "split": "validation", **val_free})
        ...
        val_search.to_csv(step_dir / f"01_threshold_search_validation_model_{variant}.csv", index=False)
        selected_threshold = select_best_threshold(val_search, beta=config.THRESHOLD_BETA)

        val_metrics = {"model_variant": variant, "split": "validation", **selected_threshold}
        ...
        val_metrics["cv_selected_param_set_id"] = selected_cv.get(variant, {}).get("param_set_id", "default_or_cv_not_run")
        validation_metric_rows.append(val_metrics)

        # Reports useful before the final test decision. These are written for every variant.
        val_topk = top_k_metrics(y_validation, val_prob, config.TOP_K_RATES)
        val_topk.insert(0, "model_variant", variant)
        val_topk.to_csv(step_dir / f"02_top_k_validation_model_{variant}.csv", index=False)

        importance = xgboost_importance(model, prepared.selected_prepared_features)
        importance.to_csv(step_dir / f"03_final_xgboost_importance_model_{variant}.csv", index=False)
        ...
        if config.ENABLE_MACHINE_LEVEL_TOP_K and val_pred is not None:
            ...
            val_machine_topk.to_csv(
                step_dir / f"02b_machine_top_k_validation_model_{variant}.csv", index=False
            )

        if config.SAVE_VALIDATION_AND_TEST_PREDICTIONS and val_pred is not None:
            val_pred.to_csv(step_dir / f"04_validation_predictions_model_{variant}.csv", index=False)

        trained_objects[variant] = {
            "model": model,
            "prepared": prepared,
            "params": params,
        ...
        print(
            f"Validation variant={variant}: AP={val_free.get('average_precision', np.nan):.4f}, "
    ...
    validation_metrics_df.to_csv(step_dir / "05_validation_selected_threshold_metrics_by_variant.csv", index=False)
    if config.AUTO_SELECT_FINAL_VARIANT_BY_VALIDATION_F2:
        final_variant = str(validation_metrics_df.iloc[0]["model_variant"])
        final_selection_reason = "selected_by_validation_f2_under_flagged_rate_constraint"
    else:
        final_variant = config.FINAL_MODEL_VARIANT
        final_selection_reason = "selected_from_config_FINAL_MODEL_VARIANT"

    if final_variant not in trained_objects:
        raise ValueError(
            f"Final model variant '{final_variant}' was not trained. Check MODEL_VARIANTS_TO_RUN in config.py."
        )

    variants_to_test = list(trained_objects.keys()) if config.EVALUATE_TEST_FOR_ALL_VARIANTS else [final_variant]
    test_metric_rows = []
    for variant in variants_to_test:
        obj = trained_objects[variant]
        threshold = float(obj["selected_threshold"]["threshold"])
        test_free = threshold_free_metrics(y_test, obj["test_probability"])
        threshold_free_rows.append({"model_variant": variant, "split": "test", **test_free})

        test_metrics = metrics_at_threshold(y_test, obj["test_probability"], threshold, beta=config.THRESHOLD_BETA)
        test_metrics = {"model_variant": variant, "split": "test", **test_metrics}
        ...
        test_topk.to_csv(step_dir / f"07_top_k_test_model_{variant}.csv", index=False)
        ...
        if config.ENABLE_MACHINE_LEVEL_TOP_K and test_pred is not None:
            ...
            test_machine_topk.to_csv(step_dir / f"07b_machine_top_k_test_model_{variant}.csv", index=False)

        if config.SAVE_VALIDATION_AND_TEST_PREDICTIONS and test_pred is not None:
            test_pred.to_csv(step_dir / f"08_test_predictions_model_{variant}.csv", index=False)

    pd.DataFrame(threshold_free_rows).to_csv(step_dir / "06_threshold_free_metrics_by_variant.csv", index=False)
    pd.DataFrame(test_metric_rows).to_csv(step_dir / "09_test_metrics_at_validation_selected_threshold.csv", index=False)

    final_obj = trained_objects[final_variant]
    metadata = {
        ...
        "xgb_params_used": final_obj["params"],
    ...
    if config.SAVE_FINAL_MODEL_ARTIFACTS:
        save_model_artifacts(
            final_obj["model"],
            final_obj["prepared"],
            models_dir / f"model_{final_variant}",
            metadata=metadata,
        )
    ...
    write_json(
        {
            ...
            "final_model_variant": final_variant,
            "final_selection_reason": final_selection_reason,
            "test_evaluated_variants": variants_to_test,
            "max_flagged_rate": config.MAX_FLAGGED_RATE,
            ...
            "notes": [
                "Final XGBoost models are trained on full training_main only.",
                "Probability threshold is selected on validation_holdout using F2 with max flagged-rate constraint.",
                "Test metrics are evaluated after threshold selection. By default only one final variant is tested.",
                "If ENABLE_MACHINE_LEVEL_TOP_K=True, validation/test machine-level top-K reports are generated by aggregating repeated snapshots per machine.",
            ],
        },
        step_dir / "00_run_summary.json",
    )
    print(f"02_train_validate_test completed. Outputs: {step_dir}")
    print(f"Final test-evaluated variant: {final_variant}")
