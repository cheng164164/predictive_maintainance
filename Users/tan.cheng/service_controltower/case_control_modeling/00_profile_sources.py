"""Step 00: profile the source data used by the modeling pipeline."""
...
def _profile_one(name: str, df: pd.DataFrame, date_col: str | None = None) -> dict:
    row = {
        ...
        "unique_machines": (
            int(df["machine_key"].nunique(dropna=True))
...
def run() -> None:
    ...
    pd.DataFrame(profile_rows).to_csv(
        step_dir / "source_profile_summary.csv", index=False
    )
        ...
        overlap_rows.append(
            {
                ...
                "machine_overlap_with_warranty": len(machines & warranty_machines),
                "warranty_machine_coverage_rate": (
                    len(machines & warranty_machines) / len(warranty_machines)
                    if warranty_machines
                    else None
                ),
            }
        )
    pd.DataFrame(overlap_rows).to_csv(
        step_dir / "source_warranty_overlap_summary.csv", index=False
    )
